...
abstract class ViperPresenter<V : ViewCallbacks, R : Router> : Presenter<V> {
   ...
   * @since 0.1.0
   */
  protected val router: R = RxViper.createRouter<R>(null, javaClass)

  private var currentRouter: R? = null
    get() = getProxy(router).get()


  protected constructor(view: V, router: R) : super(view) {
    getProxy(this.router).set(router)
  }

  protected constructor(view: V) : super(view)
   ...
   * @since 0.7.0
   */
  fun hasRouter(): Boolean = currentRouter != null
  ...
  fun takeRouter(router: R) {
    val currentRouter = this.currentRouter
    if (currentRouter !== router) {
